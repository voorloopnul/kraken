#!/usr/bin/env python3
"""Drive the Kraken Qt/QML window from a shell.

Kraken is a frameless Qt Quick window with no accessibility surface and no
...
Commands:
  launch [--binary PATH]   start Kraken, wait for its window, print pid + wid
...
WIN_NAME = "Kraken"          # QML `title:` -- the real toplevel
DEFAULT_BINARY = "target/debug/kraken"
...
def find_window(dpy):
    """Return the real toplevel. Qt also creates a 10x10 unmapped 'kraken'
...
def ensure_focus(dpy, win, timeout=5.0):
    ...
    while time.time() < deadline:
        if is_or_contains(dpy, win, focused_window(dpy)):
            return
        activate(dpy, win)
    sys.exit("Kraken never took keyboard focus -- another window is holding it, "
...
def cmd_launch(args):
    ...
    for _ in range(100):
        ...
        if w is not None:
            ...
            return
    sys.exit("timed out waiting for the Kraken window to map")
...
def cmd_quit(args):
    ...
    w = find_window(dpy)
    if w is None:
        print("no Kraken window; nothing to quit")
        return
