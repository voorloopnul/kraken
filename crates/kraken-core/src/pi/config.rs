...
/// Kraken's override for the `~/.pi` root, which covers the sessions directory
/// too — one variable that keeps a test (or a second install) off the
/// developer's real agent.
const ENV_PI_HOME: &str = "KRAKEN_PI_HOME";
...
/// else `$KRAKEN_PI_HOME/agent`, else `~/.pi/agent`.
...
mod tests {
    ...
    impl Scratch {
        fn new(name: &str) -> Self {
            ...
            let dir = std::env::temp_dir().join(format!("kraken-pi-config-{name}"));
    ...
    fn pis_own_override_names_the_directory_and_ours_names_its_root() {
        ...
        // KRAKEN_PI_HOME points at the `.pi` root, the way `~/.pi` is written.
        std::env::remove_var(ENV_AGENT_DIR);
