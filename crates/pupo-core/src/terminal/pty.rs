...
//! an environment describing the user's machine rather than the one Pupo is
//! running on.
//!
...
/// Variables naming the Python *Pupo itself* may be running on. A shell opened
/// in a workspace must not inherit any of them: with `VIRTUAL_ENV` set, `uv`
/// and `pip` resolve against Pupo's virtualenv from any directory (it outranks
/// the `.venv` sitting right there), `PYTHONPATH` makes Pupo's own sources
/// importable from the user's project, and `PYTHONHOME` points a bare `python`
/// at Pupo's runtime instead of theirs.
...
/// into the environment so that Pupo's *own* process can find its bundled Qt
...
/// What Pupo claims to be. The VT engine in [`super::vt`] implements the xterm
...
/// that describe the runtime Pupo is itself running on and the bundle it was
/// launched from.
///
...
pub struct Command {
    ...
    /// Where the child starts. The chdir happens *in the child* — Pupo's own
...
unsafe fn child_exec(
    ...
    // A session of our own. Without it the shell would share Pupo's process
    ...
    // The master is Pupo's end. A child holding it open would keep the pty
...
mod tests {
    ...
    fn app_env() -> Vec<(String, String)> {
        [
            ("VIRTUAL_ENV", "/somewhere/pupo/.venv"),
            ("PYTHONPATH", "/somewhere/pupo"),
            ("PYTHONHOME", "/somewhere/runtime"),
            ("LD_LIBRARY_PATH", "/tmp/.mount_pupo/usr/lib"),
            ("APPDIR", "/tmp/.mount_pupo"),
            ("PATH", "/somewhere/pupo/.venv/bin:/opt/homebrew/bin:/usr/bin:/bin"),
            ("EDITOR", "vim"),
        ]
        .into_iter()
    ...
    #[test]
    fn the_shell_is_not_told_about_pupos_own_python() {
    ...
    fn the_virtualenvs_bin_leaves_path_too() {
        ...
        assert!(!parts.contains(&"/somewhere/pupo/.venv/bin"));
    }
    ...
    fn an_app_outside_a_virtualenv_keeps_its_path() {
        let vars = [("PATH", "/usr/bin:/bin"), ("PYTHONPATH", "/opt/pupo/share")]
            .into_iter()
            .map(|(k, v)| (k.to_string(), v.to_string()));
        let env = scrub_env(vars);
        assert_eq!(env["PATH"], "/usr/bin:/bin");
        // PYTHONPATH still goes: in a bundle it names Pupo's own sources, which
    ...
    fn a_pty_round_trips_bytes_through_its_child() {
        ...
        while !seen.contains("pupo") && Instant::now() < deadline {
            let mut buf = [0u8; 256];
        ...
        assert!(seen.contains("pupo"), "child never echoed; saw {seen:?}");
