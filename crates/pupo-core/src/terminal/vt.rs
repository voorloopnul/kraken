...
//!
//! Kraken linked libghostty-vt over ctypes for this. Pupo linked it again, this
...
//! implementation behind Pupo and Ghostty rather than two that have to be kept
...
impl Screen {
    ...
    /// red no matter which theme Pupo is on.
    ...
    pub fn resize(&mut self, cols: usize, rows: usize) {
        ...
        // Cell pixel metrics only matter to features Pupo does not use (kitty
...
mod tests {
    ...
    fn the_window_title_comes_out_of_an_osc() {
        let mut vt = vt(10, 3);
        vt.feed(b"\x1b]0;pupo \xe2\x80\x94 build\x07");
        assert_eq!(vt.title(), "pupo — build");
        assert_eq!(vt.screen_mut().take_title().as_deref(), Some("pupo — build"));
