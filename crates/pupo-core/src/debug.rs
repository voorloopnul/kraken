...
pub fn from_environment() -> Option<Settings> {
    let path = std::env::var("PUPO_DEBUG").ok()?;
    if path.is_empty() || path == "0" {
        return None;
    }
    Some(Settings {
        path: (path != "1").then_some(path),
        trace_input: matches!(std::env::var("PUPO_DEBUG_TRACE").as_deref(), Ok("1")),
        heartbeat: std::env::var("PUPO_DEBUG_HEARTBEAT")
            .ok()
...
pub fn default_path() -> PathBuf {
    ...
    state::logs_dir().join(format!("pupo-{stamp}-{}.log", std::process::id()))
}
...
fn banner() {
    ...
    write(
        "boot",
        &format!("pupo pid={} argv={argv:?}", std::process::id()),
        false,
    );
...
mod tests {
    ...
    impl Scratch {
        fn new(name: &str) -> Self {
            ...
            let path = std::env::temp_dir().join(format!("pupo-debug-{name}.log"));
    ...
    fn the_exit_marker_is_written_on_shutdown() {
        let lock = GUARD.lock().unwrap_or_else(|e| e.into_inner());
        let path = std::env::temp_dir().join("pupo-debug-marker.log");
        let _ = std::fs::remove_file(&path);
        start(&Settings {
            path: Some(path.to_string_lossy().into_owned()),
            ..Settings::default()
    ...
    fn the_banner_names_the_process_and_the_session() {
        ...
        let text = scratch.text();
        assert!(text.contains(&format!("pupo pid={}", std::process::id())), "{text}");
    ...
    fn the_default_path_is_dated_and_pid_stamped() {
        ...
        assert!(name.starts_with("pupo-"), "{name}");
    ...
    fn the_environment_turns_tracing_on_only_when_it_says_something() {
        let restore = std::env::var("PUPO_DEBUG").ok();
        // SAFETY-adjacent: these tests hold the same lock, so no other test is
        // reading the environment while it is being changed here.
        let lock = GUARD.lock().unwrap_or_else(|e| e.into_inner());
        std::env::remove_var("PUPO_DEBUG");
        assert_eq!(from_environment(), None);
        std::env::set_var("PUPO_DEBUG", "0");
        assert_eq!(from_environment(), None);
        std::env::set_var("PUPO_DEBUG", "1");
        assert_eq!(from_environment().unwrap().path, None);
        std::env::set_var("PUPO_DEBUG", "/tmp/x.log");
        assert_eq!(
            from_environment().unwrap().path.as_deref(),
            Some("/tmp/x.log")
        );
        match restore {
            Some(value) => std::env::set_var("PUPO_DEBUG", value),
            None => std::env::remove_var("PUPO_DEBUG"),
        }
        drop(lock);
    }
}
