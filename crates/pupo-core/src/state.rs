//! Persistent app state in `~/.pupo/`: preferences and project tracking.
//!
...
/// Overridden by tests (and by `PUPO_HOME`) so state work never touches the
/// developer's own `~/.pupo`.
static ROOT: Lazy<Mutex<PathBuf>> = Lazy::new(|| Mutex::new(default_root()));

fn default_root() -> PathBuf {
    if let Ok(path) = std::env::var("PUPO_HOME") {
        if !path.is_empty() {
...
/// Point the whole module at another directory. For tests, and for `PUPO_HOME`.
...
mod tests {
    ...
    impl Scratch {
        fn new(name: &str) -> Self {
            ...
            let dir = std::env::temp_dir().join(format!("pupo-state-{name}"));
    ...
    fn the_file_round_trips_as_pretty_json_with_a_trailing_newline() {
        ...
        set("current_workspace", json!("/home/pascal/Workspace/pupo"));
