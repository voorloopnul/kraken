...
pub const SSH_ENV: &str = "PUPO_SSH";
...
/// the bundled ssh routing extension and a `PUPO_SSH` environment descriptor,
...
/// Pupo's process group and signalling that group would take Pupo down with
...
mod tests {
    ...
    fn a_local_workspace_launches_a_plain_rpc_agent() {
        let launch = Launch::new("/home/pascal/Workspace/pupo");
    ...
    #[test]
    fn the_group_signal_refuses_to_fire_at_pupos_own_group() {
    ...
    fn echo_agent(name: &str) -> PiAgent {
        let path = std::env::temp_dir().join(format!("pupo-fake-pi-{name}.sh"));
