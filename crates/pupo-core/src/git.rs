...
pub(crate) mod tests {
    ...
    impl TestRepo {
        pub fn new(name: &str) -> Self {
            let path = std::env::temp_dir().join(format!("pupo-git-{name}-{}", std::process::id()));
    ...
    fn a_local_runner_puts_git_in_the_workspace_with_dash_c() {
        let runner = GitRunner::local("/home/pascal/Workspace/pupo");
        assert_eq!(
            runner.argv(&args(&["status", "-z"])),
            vec!["git", "-C", "/home/pascal/Workspace/pupo", "status", "-z"]
        );
    }
    ...
    fn a_command_that_does_not_exist_is_a_missing_answer_not_a_panic() {
        assert_eq!(
            run_argv(&args(&["pupo-no-such-binary", "x"]), QUICK),
            None
        );
    ...
    fn the_folder_line_shortens_the_home_folder() {
        assert_eq!(
            home_relative_to("/Users/x/Workspace/pupo", "/Users/x"),
            "~/Workspace/pupo"
        );
        assert_eq!(home_relative_to("/Users/x", "/Users/x"), "~");
        assert_eq!(home_relative_to("/opt/pupo", "/Users/x"), "/opt/pupo");
        // A folder that merely starts with the same characters is not inside it.
        assert_eq!(
            home_relative_to("/Users/xylophone/pupo", "/Users/x"),
            "/Users/xylophone/pupo"
        );
    }
    ...
    fn the_branch_is_read_out_of_head_without_spawning_git() {
        let dir = std::env::temp_dir().join(format!("pupo-head-{}", std::process::id()));
    ...
    fn a_worktrees_gitdir_file_is_followed_to_its_head() {
        let root = std::env::temp_dir().join(format!("pupo-worktree-{}", std::process::id()));
    ...
    fn a_folder_that_is_not_a_repository_says_that_instead() {
        let dir = std::env::temp_dir().join(format!("pupo-not-a-repo-{}", std::process::id()));
        let _ = fs::remove_dir_all(&dir);
