...
Item {
    ...
    // Normally the file list is the only thing that scrolls, and it is the only
    // thing that gives up height. Below the point where it has none left to
    // give — the dock is floored at MIN_PANEL_HEIGHT, which is shorter than the
    // composer alone — the whole pane scrolls instead, rather than sliding the
    // composer up over the summary and squeezing the list out of existence.
    Flickable {
        id: pane
        anchors.fill: parent
        clip: true
        ...
        contentWidth: width
        // `list.y` is fixed by the anchors above it, so this asks only for the
        // height the form cannot do without: the composer whole, and enough of
        // the list to still read as a list.
        contentHeight: Math.max(height, list.y + 66 + 8 + composer.height + 10)
        // Nothing to scroll is nothing to catch: an idle Flickable would still
        // swallow the drag that selects text in the message box.
