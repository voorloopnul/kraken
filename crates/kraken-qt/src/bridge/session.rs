...
//! the composer is holding. `kraken_core::pi::controller` does the talking to pi
//! and `kraken_core::chat` does the rendering; what is left here is the seam —
...
use kraken_core::chat::formatting::replay_footers;
use kraken_core::chat::markdown;
use kraken_core::chat::transcript::{thinking_summary, Attachment, Block, FLUSH_DUTY, FLUSH_INTERVAL_MS};
use kraken_core::pi::config;
use kraken_core::pi::controller::{available_levels, RequestKind, SessionController, Signal};
...
impl SessionBridge {
    pub fn new() -> Self {
        Self {
            theme_name: kraken_core::theme::DEFAULT_THEME.to_string(),
            font_size: kraken_core::typography::chat::DEFAULT_SIZE,
    ...
    fn set_font_size(&mut self, size: i32) {
        let size = kraken_core::typography::chat::clamp(size);
