...
//!
//! The arrangement itself lives in `kraken_core::dock`, which knows the rules and
...
use kraken_core::dock::{ColumnRect, Dock, DropTarget};
use kraken_core::state;
use qmetaobject::*;
...
pub struct DockModel {
    ...
    /// either side can give. The rule itself is in `kraken_core::dock`.
...
impl DockModel {
    ...
    fn preferred_width(&self, key: QString) -> i32 {
        kraken_core::dock::preferred_width(&key.to_string())
    }
    ...
    fn column_width(&self, key: QString) -> i32 {
        ...
        if !kraken_core::dock::is_resizable(&key) {
            return kraken_core::dock::preferred_width(&key);
        }
        self.widths
            .get(&key)
            .copied()
            .unwrap_or_else(|| kraken_core::dock::preferred_width(&key))
            .max(kraken_core::dock::min_width(&key))
    }

    fn min_width(&self, key: QString) -> i32 {
        kraken_core::dock::min_width(&key.to_string())
    }

    fn resizable(&self, key: QString) -> bool {
        kraken_core::dock::is_resizable(&key.to_string())
    }
    ...
    fn set_column_width(&mut self, key: QString, width: i32) {
        ...
        let width = width.max(kraken_core::dock::min_width(&key));
    ...
    fn column_split(&self, key: QString) -> f64 {
        self.splits
            .get(&key.to_string())
            .copied()
            .unwrap_or(kraken_core::dock::DEFAULT_SPLIT)
    }
    ...
    fn min_panel_height(&self) -> i32 {
        kraken_core::dock::MIN_PANEL_HEIGHT
    }

    fn stack_rows(&self, height: i32, fraction: f64, count: i32, gap: i32) -> QString {
        rows_json(kraken_core::dock::stack_rows(
            height,
            fraction,
    ...
    fn stack_bands(&self, height: i32, fraction: f64, count: i32, gap: i32) -> QString {
        rows_json(kraken_core::dock::stack_bands(
            height,
            fraction,
            count.max(0) as usize,
            gap,
    ...
    fn resize_step(
        ...
        kraken_core::dock::resize_step(
            travel,
            kraken_core::dock::Side { width: left, min: left_min },
            kraken_core::dock::Side { width: right, min: right_min },
        )
    }
    ...
    fn fit_columns(&self, available: i32, columns: QString) -> QString {
        ...
        let sides: Vec<kraken_core::dock::Side> = parsed
            .iter()
            .map(|value| {
                let number = |key: &str| {
                    value
                ...
                kraken_core::dock::Side {
                    width: number("width"),
                    min: number("min"),
                }
            })
            .collect();
        let fitted = kraken_core::dock::fit_columns(
            available,
            kraken_core::dock::min_width("center"),
            &sides,
        );
    ...
    fn panels_that_do_not_fit(&self, available: i32) -> QString {
        ...
        let mins: Vec<i32> = side
            ...
            .map(|keys| {
                keys.first()
                    .map_or(0, |key| kraken_core::dock::min_width(key))
            })
            .collect();

        let fits = kraken_core::dock::side_columns_that_fit(available, history_visible, &mins);
    ...
    fn update_drag(&mut self, x: f64, y: f64) {
        ...
        if !self.drag_active {
            ...
            if travelled < f64::from(kraken_core::dock::DRAG_THRESHOLD) {
                return;
            }
