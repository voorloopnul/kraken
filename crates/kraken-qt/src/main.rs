//! Kraken — a Qt/QML desktop front end for the Pi coding agent.
...
fn install_ssh_extension() {
    write_ssh_extension(&kraken_core::pi::rpc::ssh_extension_path());
}
...
mod tests {
    ...
    fn scratch(name: &str) -> std::path::PathBuf {
        let dir = std::env::temp_dir().join(format!("kraken-ext-{name}"));
    ...
    fn the_extension_reads_the_variable_the_launcher_sets() {
        let reads = format!("process.env.{}", kraken_core::pi::rpc::SSH_ENV);
        assert!(
            SSH_EXTENSION.contains(&reads),
            "the bundled extension never reads {}",
            kraken_core::pi::rpc::SSH_ENV
        );
    }
